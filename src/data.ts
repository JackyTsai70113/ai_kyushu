...
export const ITINERARY_DATA: ItineraryDay[] = [
  {
    ...
    note: "住：南阿蘇 CAN LI ASSO。第一天從熊本機場下飛機開始，取車後先到 MERUKORO PAN-KOUBOU 買麵包，再上草千里 Green Park 吃中餐，最後高森補給與入住。砂千里之濱因 2026/7 火山警戒與緊急工事封閉，本趟不安排（詳見當天底部備註）。大雨時不去白川水源，改走熊本機場 → 南阿蘇室內放電 → 阿蘇補給 → 入住。草帽雕像可選短停：娜美像（俵山交流館 萌の里），雨小且取車順利才拍。",
